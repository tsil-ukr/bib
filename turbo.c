...
extern логічне __турбо__запустити_інтернет_обслуговувач(
    Турбіна* турбіна,
    позитивне іа,
    позитивне порт,
    ТурбоОбробникЗапускуІнтернетОбслуговувача обробник_запуску,
  ...
  інтернет_обслуговувач->обробник_запуску = обробник_запуску;
  інтернет_обслуговувач->обробник_підключення_клієнта =
      обробник_підключення_клієнта;
  ...
  обробник_запуску(турбіна, інтернет_обслуговувач, аргумент, іа, порт);
...
extern void __турбо__запустити_турбіну(Турбіна* турбіна,
  ...
  while (1) {
    ...
    if (дані_події->тип == ТипПодіїІнтернетОбслуговувачКлієнтПідключено) {
      обробити_інтернет_обслуговувач_підключено_клієнт(
          турбіна, дані_події->обʼєкт.інтернет_обслуговувач, cqe->res);
    } else if (дані_події->тип == ТипПодіїІнтернетЗвʼязокПідключено) {
